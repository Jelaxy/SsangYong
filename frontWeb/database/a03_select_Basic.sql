...
SELECT deptno, avg(sal) avgsal, min(sal) misal, max(sal) masal /* 그룹할 컬럼, 함수명(컬럼명)*/
FROM emp
GROUP BY deptno;
...
FROM emp;
SELECT a.*, sal + bonus from(
							SELECT ename, sal, sal*0.2 bonus
							FROM emp;) a; -- subquery 활용
...
FROM emp;
-- 문자열 안에 연산을 처리할 때는 ()로 우선연산을 처리해서 활용한다.
SELECT deptno FROM emp;
-- 중복되는 데이터를 제거하고 출력할 때 사용되는 distinct
SELECT DISTINCT deptno FROM emp;
SELECT * FROM emp;
-- job과 mgr 각각 중복데이터를 제거한 데이터 내용을 리스트하세요
SELECT DISTINCT job FROM emp;
SELECT DISTINCT mgr FROM emp;
SELECT * FROM emp;
-- distinct 컬럼1, 컬럼2
-- 컬럼1과 컬럼2 두개 모두를 비교해서 다른 데이터 로딩
