...
2. 형변환 유형
	1) 묵시적 데이터 타입의 변환
		...
		적절한 형변환 하는 함수를 적용하는 것이 더 효율적이다.
...
3. 출력할형식1(날짜 ==> 문자열)
	...
	3) AD/A.D AD/BC 표기
		둘 중에 하나만 표기만 하면 해당 연도의 A.D/B.C 인지를 구부낳여 데이터 처리가 된다.
	4) Q  분기 1/4분기, 2/4분기..
	5) MM/MONTH/MON 월표기
...
--4. `EMP` 테이블에서 `ENAME`이 'SMITH'인 직원의 
--입사일(`HIREDATE`)을 'YYYY년 MM월 DD일' 형식으로 출력하세요.
SELECT ENAME, 
TO_CHAR(HIREDATE, 'YYYY"년 "MM"월 "DD"일"') 
FROM EMP WHERE ENAME = 'SMITH';
--5. `EMP` 테이블에서 `SAL`이 3000 이상인 직원의 
--`ENAME`, `SAL`, `HIREDATE`를 출력하되, 
--`HIREDATE`는 'YYYY년 Q분기' 형식으로 출력하세요.
SELECT ENAME, SAL, 
TO_CHAR(HIREDATE, 'YYYY"년 "Q"분기"') FROM EMP
WHERE SAL >= 3000;
--6. `EMP` 테이블에서 모든 직원의 `ENAME`, `SAL`,
-- `HIREDATE`를 출력하되, `HIREDATE`는 'YY년 MM월 DD일에 Q/4분기' 형식으로 출력하세요.
SELECT ENAME, SAL, TO_CHAR(HIREDATE,
'YY"년 "MM"월 "DD"일에 "Q"/4분기') FROM EMP;
--7. `EMP` 테이블에서 `ENAME`이 'ALLEN'인 직원의 `ENAME`, `SAL`, `HIREDATE`를 출력하되, `HIREDATE`는 'HH24시 MI분 SS초' 형식으로 출력하세요.
SELECT ENAME, SAL, TO_CHAR(HIREDATE, 
'HH24"시 "MI"분 "SS"초"') FROM EMP 
WHERE ENAME = 'ALLEN';
--8. `EMP` 테이블에서 `ENAME`이 'KING'인 직원의 `ENAME`, `SAL`, `HIREDATE`를 출력하되, `HIREDATE`는 'DD일 DAY YYYY년 Q/4분기' 형식으로 출력하세요.
 SELECT ENAME, SAL, TO_CHAR(HIREDATE, 
'DD"일 "DAY" "YYYY"년 "Q"/4분기"') 
FROM EMP WHERE ENAME = 'KING';
--9. 현재 시간을 'HH24시 MI분 SS초' 형식으로 출력하세요.
SELECT TO_CHAR(SYSDATE, 'HH24"시 "MI"분 "SS"초"') FROM dual;
--10. `EMP` 테이블에서 모든 직원의 `ENAME`, `SAL`, 
--`HIREDATE`를 출력하되, `HIREDATE`는 
--'YYYY년 MM월 DD일 A.M./P.M.' 형식으로 출력하세요.
SELECT ENAME, SAL, TO_CHAR(HIREDATE,
'YYYY"년 "MM"월 "DD"일 "AM"') msg FROM EMP;
