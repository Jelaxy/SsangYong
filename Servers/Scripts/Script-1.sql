...
# SQL 함수의 유형
1. 단일행 함수 : 테이블에 저장되어 잇는 개별 행을 대상으로 함수를 적용하여 하나의 결과를 반환하는 함수
	-데이터 값을 조작하는 데 주로 사용
...
-- 1. initcap() : 문자의 첫번쨰 문자만 대문자로 변환
SELECT ename, INITCAP(ename), job, INITCAP(job)
FROM emp;
-- ex) ||연결연산자, initcap()를 활용하여
--	사원명은 Smith is a Clerk! 표현하세요
SELECT ename, INITCAP(ename) ||' is a '|| job, INITCAP(job)
FROM emp;
