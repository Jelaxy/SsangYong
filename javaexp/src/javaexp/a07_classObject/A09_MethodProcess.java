...
class Calculator2{
	...
	// ex) 매개변수를 받아서 두 개의 값을 입력받아, 해당 두 개의
	//     구구단 단수의 결과를 출력하세요
	//     3단 7단 출력
	public void getGugu(int grade1, int grade2) {
		System.out.println("#"+grade1+"단#");
		for(int cnt=1;cnt<=9;cnt++) {
			System.out.println(grade1+"x:"+cnt+"="+(grade1*cnt)); 
		}System.out.println("#"+grade2+"단#");
		for(int cnt=1;cnt<=9;cnt++) {
			System.out.println(grade2+"x:"+cnt+"="+(grade2*cnt)); 
			}
		}
	...
	public void callBuyMaxCnt(int price, int pcnt) {
		System.out.println("물건의 단가:"+price);
		System.out.println("최대 구매 갯수:"+pcnt);
		for(int cnt=1;cnt<=pcnt;cnt++) {
			System.out.println("1개 구매시:"+price*pcnt);
		}
	}
