...
public class A02_String {
	...
	public static void main(String[] args) {
		...
		2. 문자열의 비교
		  1) 주소값 비교 원칙
		  	stack/heap
		  	모든 객체들은 heap영역에 실제 객체를 두고, 그 heap 영역의 위치를(주소값/참조변수) stack에 할당하여 ㅏ용한다.
		  	String name1 = "홍길동";
		  	String name2 = "홍길동";
		  	String name3 = new String("홍길동");
		  	String name4 = new String("홍길동");
		  	
		  	== : stack의 데이터를 비교하여 boolean값을 가져오는 것이다
		  	new 무조건 객체가 다른 위치(heap)에 생성이 된다.
		  	
		  	처음에는 "홍길동"을 만들고 특정한 heap영역에 위치에 할당
		  	그 다음 "홍길동" 처음에 만들었던 주소값을 그대로 사용해서 변수에 할당
		  	
		  	주의) 자바는 대부분 선언을 해서 문자열을 사용하는 것이 아니라,
		  	입력을 받든지, 파일에 있는 데이터를 로딩하든지, 네트워크상으로 데이터를 가져오든지
		  	DB server에서 데이터를 가져오면 명시적 new String()있지는 않지만 내부적으로 처리되기에 주소값이 다르다고 봐야한다.
		...
		3. String의 기능 메서드
		  1) 생성자
		  	new String()
		  	new String(byte[]) **
		  		최종프로젝트에 활용(아이디를 잊었을 때, 임시 비밀번호 생성 회사에서 최초 사번과 임시비밀번호 통해서 발급)
		  	new String(char[])
		  	new String("문자열")
		 */
		char [] arr = {'a','e','a','e'};
		...
		// 숫자 데이터 48~57, 대문자의 범위 65~90, 소문자의 범위 97~122
		// 65~90 ==> (int)(Math.random()*A+B)
		// 65~90 ==> (int)(Math.random()*26+65)
		for(int idx=0; idx<arr1.length;idx++) {
			byte num=(byte)(Math.random()*26+65);
			System.out.println(idx+"번째 임의의 수"+num);
