...
public class A09_Final {
	...
	public static void main(String[] args) {
		...
		1. 필드에서 final
			객체단위 final : 
				하나의 클래스에서 생성되는 여러개의 객체에 객체별로 변경하지 않는 상수.
				class Person{
				...
				Person.KIND : 객체 생성없어도 사용할 수 있고
				Person.KIND="백인" : final이기에 변경할 수 있음
				객체 공유 즉 class 상수
				// 객체별로 초기화는 가능하나
				Person p01 = new Person("홍길동");
				Person p02 = new Person("김길동");
				p01.name="오길동"; (x) // 한번 할당된 데이터는 변경하지 못함
				p02.name="마길동"; (x)
...
class Book{
	void showPrice() {
		System.out.println("가격알려줘");
	}
	final void showAll() {
		System.out.println("책 정보 보여줘!");
	}
}
class SubBook extends Book{
	// final 메서드라 오버라이딩이 안뜸
}