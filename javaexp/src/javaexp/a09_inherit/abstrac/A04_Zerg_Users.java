...
class ZergUser{
	...
	// ArrayList<Larba>에 하나씩 추가해 가능과정 처리
	public void makeUnit(Larba larba) {
		units.add(larba);
		System.out.println(color+"유닛 "+larba.getUnit()+"을 생산하다");
	}
	public void allAttack() {
		System.out.println(color+"모든 유닛이 공격을 합니다");
		if(units.size()>0) {
			System.out.println(color+"전체 유닛"+units.size()+"공격합니다");
			for(Larba lb:units) {
				System.out.println("\t");
				lb.move(); // 공통메서드
				System.out.println("\t");
			...
			System.out.println("공격할 유닛이 없네요");
		}
	}
