...
public class A01_StrarcraftWorld {

	public static void main(String[] args) {
		// 추상 클래스는 혼자 객체를 생성하지 못한다.
		// 추상 클래스의 목적이 하위 클래스의 공통기능 선언과
		// 		다형성(메서드 재정의를 통해)이기 때문에
		// 추상 클래스는 다형성 처리할 메서드를 강제성 있게 처리할 목적이기에
		// 실제 클래스가 필요없게 만든다
//		Larba lb = new Larba("드론","빨강"); 에러 발생 <== 실체 내용 없음
		Larba lb100 = new Dron(); // 추상 = 하위 실제();
		lb100.move(); // 상속해서 사용할  공통 내용
		lb100.getUnit(); // 상속해서 사용할  공통 내용
		lb100.attack(); // 상송해서 사용할 다형성 처리 내용. 재정의 강제하고 있다.
		...
		# 추상클래스의 목적
		  1) 기능적인 통일성을 가진 상위클래스를 상속 받게하기 위한 것이다.
		  	- 기능적 통일성
		  		공통 필드/메서드 : 그대로 재활용
		  		이름은 같지만 다양한 기능을 처리하는 메서드를 강제화한다.***(오버로딩을 강제화)
		# 추상클래스를 통한 실제 클래스 만들기
			abstract class Larba {
				공통 필드 : 유닛종류, 색상
				공통 생성자: 매개변수 1개 : 하위에 반드시 호출
				공통 메서드 : 유닉이 이동한다.
...
class Dron extends Larba{
	...
	public void attack() {
		...
		System.out.println(getUnit()+" 공격력5로 미약하게 공격");
	}
...
class Zerggling extends Larba{
	...
	@Override
	public void attack() {
		System.out.println(getUnit()+" 초기공격유닉으로 공격력 5로 빠르게 공격");
	}
...
class Hydralisk extends Larba{
	...
	@Override
	public void attack() {
		System.out.println(getUnit()+" 중대형 공격 유닉으로 공격력 15로 원거리 공격");
	}
	@Override
	public void move() {
		super.move();
