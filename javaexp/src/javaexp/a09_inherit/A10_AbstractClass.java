...
public class A10_AbstractClass {
	...
	public static void main(String[] args) {
		...
		2. 추상클래스
			...
			==> 추상메서드가 하나라도 있으면 추상 클래스가 된다. 공통내용(공통기능 메서드)+개별적으로 다른 기능 처리(추상메서드)
		3. 특징
		  1) 일반 클래스와 거의 유사하고 추상 메서드만 있다.
		  2) 추상 메서드는 하위에서 반드시 재정의해야 하는 부분(다형성 처리 필요)
		  	- 하위클래스에서 추상메서드 강제화(컴파일에러)
		  3) 추상 클래스는 단독으로 객체 생성을 하지 못한다.
		  	 추상 클래스 = new 상속받은 실체 객체(); 로만 객체 생성.
		  4) 단계별 진행 내용
		    # 1단계
		    ...
		    4) 위 추상클래스와 상속실제 클래스를 1:1관계 포함한 처리
		    	...
		    	ex) 마트에서 하나의 구매품 사기
		    		Mart
		    		Product product; (필드선언)
		    		Product를 추상클래스(prodInfo()추상메서드)로해서
		    		하위에 여러가지 상속 판매물건 Fruit, Food, Icecream
		    ...
		    5) 위 추상클래스와 상속실제 클래스를 1:다관계를 포함한 처리
		    	...
		    	ex) 여러개의 물건을 구매할 수 있는 마트
		    		Mart
		    			ArrayList<Product>; (필드선언)
		    			buyProduct(Product prod)
		    				buyList.add(prod); //구매물품추가
		    			showBuyList()
		    				name 마트에서 구매한 내역
		    				int tot=0;
