...
3. 활용
	1) 사용자 정의 예외는 일반적으로 api에서 지원되는 예외와
		더불어 특정한 경우에 예외를 던져 해당 정의된 예외 클래스에서 만들어지는 객체로 처리하는 경우를 말한다.
	2) 사용자 정의 예외 처리 예시
		- 아이디 패스워드에 입력 유효성 처리
		- 회원 가입 아이디 입력 유효성
		- 네트웤 서버 접속시 접속 불가능할 때
4. 처리 순서
	1) 필요로 하는 내용을 요구사항 정의서에서 명확히 정리
	2) 사용자 정의 클래스에서 필요한 메시지, 기능내용을 구분해서 정리
	3) 클래스 정의
	4) 해당 내용이 처리될 조건 및 프로세스 정리
	5) try{}catch{}문 예외 처리 및 사용자 정의 예외 객체 생성
5. 사용예제
	- 회원가입 아이디 입력 유효성(8~16)범위 안에서 입력 처리
		범위에 맞지 않을 때, 다시 입력하게 처리
	- 메시지 : 입력한 아이디 글자는 
		아이디는 8~16자 사이 입력가능합니다
	  기능내용 : getMessage()로 위 메시지내용 재정의 처리
	- 위 내용 기반 예외 클래스 정의
	- 해당 내용이 처리될 조건 및 프로세스 정리
		Scanner로 입력시 글자가 8~16이 아닐때 예외 던지기
