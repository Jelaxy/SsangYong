	...
	public static void main(String[] args) {
		...
		2. 배열의 문제점
			...
			2) 객체 삭제했을 때, 객체를 저장하는 문제 발생
				==> 삭제하면 낱알빠진 옥수수처럼 해당 내용만 null 처리된다
				==> 추가 하지 못하는 단점
			3) 객체들을 효율적으로 동적으로 추가, 삭제, 검색 할 수 있음
			4) 인터페이스를 통해서 정형화된 방법으로 다양한 컬렉션 클래스 사용 및 확장성있게 활용가능
			ex) List<Person> plist = new ArrayList<Person>();
			
		# 컬렉션 프레임워크의 주요 인터페이스
		1. 인터페이스와 실제 객체들
			1) List - ArrayList, Vector, LinkedList
					인터페이스를 상속받아서 사용자 정의 실제 클래스를 정의가 가능하다.
