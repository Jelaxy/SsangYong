...
# 클래스 선언과 접근 제어자
1. public class 클래스명
	클래스명으로 객체를 선언할 때, 외부 패키지에서 접근 가능을 확인하는 부분
 	1) 같은 패키지에서 호출하는 경우
 		class 클래스명
 		클래스명 참조명; (o)
 	...
 	같은 패키지인경우
 		Person p01; (o)
 	외부 패키지인경우 : import를 전제 조건/import하지 않을 때는
 					패키지명.클래스명 참조명;
 		Person p01; (o)
# class 선언관련해서 접근 제어자는 객체의 선언에 영향을 미친다
주의) 이 접근 제어자가 통과 되지 않을 때는
	 하위 종속된 필드, 생성자, 메서드 등의 접근범위가 public이더라도 접근할 수 없다.
2. 생성자에 대한 접근 제한
  0) 위 선언 접근제어자가 접근 범위에 있을 때를 전제 조건으로 한다.
  1) 생성자가 선언되지 않을 댸는 기본 접근 제한자가 public으로 설정이 되어 있어서 외부 패키지에서도 접근이 가능하다.
  	 public class Person{}
  	 Person p01 = new Person(); 외부 패키지에서 생성자를 호출 가능한 이유가 위 Person클래스 안에 default 생성자
  	 public Person(){}이 포함되어 있기 때문이다.
  2) 생성자가 default 접근제어자 일 때,
  	 Person(){}
  	 같은 패키지일 때
  	 	Person p01; (o)
  	 	p01 = new Person(); (o)
 ...
 */
	...
	// 접근 제어자의 목적은 은닉성
	// 기타 참조변수.필드, 참조변수.메서드는 아래 내용에 따라 접근이 된다
