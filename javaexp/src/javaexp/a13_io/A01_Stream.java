	...
	public static void main(String[] args) {
		...
		3. InputStream을 통해 입력 데이터를 OutputStream으로
		 	...
		 	2) 기본 입력 Stream 객체는 다음과 같이 데이터를 처리한다.
		 		...
		 		is1.read(bytes);
		 		문자(영문)으로 10자를 입력해서 해당 입력 데이터가 bytes라는 배욜에 코드값이 입력된다.
		 		for(byte b:bytes){
		 			(char)b; 입력된 문자 하나씩 가져와서 처리가능
		 		}
		 		==> Scanenr는 이러한 Stream객체를 포함하여 데이터를 문자열까지 입력받아서 효율적으로 사용할 수 있게 객체로 만듦
		 	3) char 형변환 처리
		 		char ch01 =(char)data; // 코드값을 char문자로 변환해서 출력
		 		
		 		System.out.prinln("입력한 문자 출력:"+ch01);
