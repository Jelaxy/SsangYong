...
public class A10_UserException {
	...
	public static void main(String[] args) {
		...
		/*
		# 사용자 정의 예외 처리
		1. 개발자가 직접적으로 사용할 예외 처리 클래스를 선언하여,
		2. 필요한 경우, 조건문이 프로세스 처리 라인에 의해
			throw new 사용자정의클래스() 형태로 예외를 처리하는 것을 말한다.
		3. 기본 형식
			class 클래스명 extends Exception{
				// 생성자 선언
				// super("문자열 데이터") : 상위 문자열을 입력받는 생성자를
				// 						 호출하여 데이터를 전송하면
				public String getMessage(){ 재정의 하는 메서드(선택 옵션) }
...
//					throw new User01Exception(cnt+"번에서 예외발생");
					...
					throw new User01Exception("특정 라인에서 예외발생");
//				}
//			}
...
class User01Exception extends Exception{
	...
	public String getMessage() {
		...
		return "재정의 된 메서드 호출:"+msg;
	}
	// 추가 기능 메서드 처리
	public void call01() {
		System.out.println("내기 만든 예외 처리 객체^^!!");
	}
