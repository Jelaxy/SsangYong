...
[1단계:코드] 2. 배열에 저장된 동물 이름을 출력하세요.
    --%>
   <% request.setAttribute("animal", new String[]{"쥐","소","호랑이","토끼"}); %>
   <table class="table table-striped table-hover">
          <tbody>
               <tr  class="text-center">
               <c:forEach var="aniArry" items="${animal}">
                 <td>${aniArry}</td>
               </c:forEach>
               </tr>
          </tbody>
         ...
         <thead class="table-success">
          ...
          <tbody>
               <tr  class="text-center">
                 ...
                 <td>
                 <c:choose>
                    <c:when test="${empty param.age}">나이를 입력하세요</c:when>
        ...
        <nav class="navbar navbar-expand-sm bg-dark navbar-dark">
          ...
          <form method="post"  class="d-flex align-items-center" >
               <input type="text" class="form-control me-2" 
                 id="id" placeholder="비밀번호 입력" value="${param.id}" name="id"  aria-label="Search">
               <input type="text" class="form-control me-2" 
                 id="pass" placeholder="비밀번호 입력" value="${param.pass}" name="pass"  aria-label="Search">
