...
<%--
# ajax
1. 비동기적으로 서버와의 통신을 javascript로 처리하여 홤녀 전화없이 데이터를  받아와서 처리하는 방식을 말한다.
2. 동기방식과 비동기 방식
	1) 동기: 서버와 클라이언트가 같은 속도로 연계하는 것을 말한다.
		- 편지를 우체통에 넣고 답장올 때까지 기다리고 오면 다른 일을 하는 것을 말한다.
		- 웹 화면에서는 form 데이터를 입력하고, submit 후 다시 화면이 전환되면 서버에서 만들어진 화면을 사용하는 경우
		- 파일 업로드시 파일을 서버에 다 올려서 해당 파일정보와 실제 파일이 있을 때, 다운로드 하거나 이미지를 볼 수 있게 하는 것 등
	2) 비동기 : 서버와 클라이언트가 같은 속도로 연계하지 않음
		-편지를 우체통에 넣기만 하고 다른일을 하다가 오면 처리하는 것을 말한다.
		- 검색처리하고, 다른 가능을 처리하면서 검색이 완료되았을 때, 화면에 로딩되어 보이게 하는 것을 말한다.
		- 비동기식으로 처리하면 해당 비동기 처리과정의 이벤트 처리와 이에 맞는 이벤트 핸들러 함수를 선언하여야 한다.
3. 주요 핵심 코드
	1) XMLHttpRequest
	...
	5) 비동기식일 때, 처리하는 이벤트 및 이벤트 핸들러 함수
		...
		xhr.status : http 응답 코드 200~299(정상적으로 받음)
			일반적을 200으로 설정 안에서 정상적으로 다 데이터가 있을 때 처리한다.
		}
 --%>
