...
3. 기본 객체들
	...
	4) param.Values: 요청 파라미터의 <파라미터 이름, 값배열> 매핑을 저장한 Map객체
		&price=3000&price=2000&price=100
		${param.Values.price[0]}
		${param.Values.price[1]}
		${param.Values.price[2]}
	5) header : 요청 정보의 <헤더이름, 값> 매핑을 저장한 Map 객체
	6) headerValues: 요청 정보의 <헤더이름, 값 배열> 매핑을 저장한 Map 객체
	7) cookie : 쿠키이름, Cookie 매핑을 저장한 Map객체
	8) initParam : 초기화파라미터의 <이름,값>매핑을 저장한 Map객체
...
<body>
    <div class="container mt-3">
    	...
    	<%-- 숫자형의 경우 자동 형변환되어 연산처리되는 것을 알 수 있다. --%>
    	<h2>${paramValues.price[0]
    			+paramValues.price[1]
    			+paramValues.price[2]}</h2>
		...
		<form action="" method="post">
            ...
            <input type="text" class="form-control" 
      	     id="sname" placeholder="학생명 입력" name="sname">
         	</div>
         	<div class="mb-3 mt-3">
            <label for="kor">국어:</label>
            <input type="number" class="form-control" 
      	     id="kor" placeholder="국어 입력" name="point">
         	</div>
         	<div class="mb-3 mt-3">
            <label for="eng">영어:</label>
            <input type="number" class="form-control" 
      	     id="eng" placeholder="영어 입력" name="point">
         	</div>      
         	<div class="mb-3 mt-3">
            <label for="math">수학:</label>
            <input type="number" class="form-control" 
      	     id="math" placeholder="영어 입력" name="point">
         	</div>
         	<button type="submit" class="btn btn-primary">등록</button>
     	</form>
		<table class="table table-striped table-hover">
		    ...
		    <tbody>
			   	<tr  class="text-center">
			        ...
			        <td>${paramValues.point[0]+paramValues.point[1]+paramValues.point[2]}</td>
			        <td>${(paramValues.point[0]+paramValues.point[1]+paramValues.point[2])/3 }</td>
			   	</tr>
		 	</tbody>
		</table>
    </div>
