...
2. 실무 프로그래
	...
	ex) 검색: 
		...
		부서명[select]
			...
			10 인사
			20 재무
			DB에서 dao로 불러온 데이터를 선택해서 검색할 수 있게 처리해야 한다.
			==> 등록/수정/석제 시에도 위 dao에서 불러온 데이터 기준으로 선택해서 처리함으로 데이터의 무결성/일관성에 문제가 없이 처리된다.
...
<%--
ex) select의 option 내용에 맞는 부서 정보를 배열로 선언하고 출력하세요 --%>
<%
String [] dNames = {"인사","총무","경영지원","영업"};
