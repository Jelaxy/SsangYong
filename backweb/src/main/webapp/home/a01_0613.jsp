...
3. JSP 조건문을 사용할 때, 화면 분리나 로직 분리에 활용되는데, 주로 어떤 상황에서 사용되는가?
...
답: A, B, C
B : 입력을 해서 ==> DB를 확인 ==> 권한 내용에 따라 메뉴접근 처리
ex) 로그인/아이디/패스워드로 인증을 받아, DB에 권한을 가져와서 이 권한에 따라 특정 메뉴를 보이게 하거나 보이지 않게 처리
// 권한에 따라 클릭시 해당 메뉴를 들어가게 하거나 접근 불가능하게 처리
...
답: B
A, B, C, D 모두 다 답이라 볼 수 있다.
B가 가장효율적이고 , A도 가능은 하나
C, D 전제 조건이 있으면 답으로 볼 수 잇다.

for(String fruit:arry){

}
for(int idx=0;idz<arry.length;idx++){
	arry[idx]
}
<%=arry[3]%>
...
12. JSP scriptlet을 사용하여 조건문을 구현하려고 한다. 다음 중 올바른 방법은?
   a) `<% if(조건) { %> 내용 <% } else { %> 내용 <% } %>`
   b) `<% if(조건) { %> 내용 <% } %>`
...
15. JSP scriptlet을 사용하여 임의의 수에 따라 메시지를 출력하려고 한다. 다음 중 올바른 방법은?
...
17. JSP scriptlet을 사용하여 학생의 성적 정보를 입력하고 평균을 계산하려고 한다. 올바른 방법은?
    a)
    ```jsp
    <form>
        <% for(int i=1; i<=5; i++) { %>
    ...
    <form>
        ...
        for(int i=1; i<=5; i++) {
            ...
            sum += score;
        %>
            <input type="text" name="score<%= i %>" value="<%= score %>" />
            <% if(i != 5) { %>
                <br />
            <% } %>
        <% } %>
 ...
 <% if(i != 5) { %>
    ...
    <form>
        ...
        for(int i=1; i<=5; i++) {
            ...
            sum += score;
        %>
            <input type="text" name="score<%= i %>" value="<%= score %>" /><br />
        <% } %>
        <input type="submit" value="계산" />
        평균: <%= sum/5 %>
    </form>
